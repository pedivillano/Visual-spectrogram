class AutoPlay {
  ...
  tryParseToken(token) {
    ...
    const numberDotOctaveRegex = /^([1-7])\.(\d+)$/;
    const numberDotOctaveMatch = token.match(numberDotOctaveRegex);
    if (numberDotOctaveMatch) {
      const noteNum = numberDotOctaveMatch[1];
      const zone = parseInt(numberDotOctaveMatch[2]);
      if (this.noteMap[noteNum] && this.octaveOffset[zone] !== undefined) {
        const note = this.noteMap[noteNum];
        const octave = 4 + this.octaveOffset[zone];
        return { note: note, octave: octave };
      }
    }
