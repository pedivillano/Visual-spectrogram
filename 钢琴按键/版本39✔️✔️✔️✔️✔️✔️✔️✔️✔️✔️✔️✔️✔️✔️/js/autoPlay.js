class AutoPlay {
  ...
  async play(notes, onNotePlayed = null, speed = 500, noteDurations = null) {
    if (this.isPlaying) {
      await this.stop();
    }
    ...
    return new Promise((resolve) => {
      const playNextNote = () => {
        ...
        let duration = speed / 1000 * 0.8;

        if (noteDurations && noteDurations[this.currentNoteIndex]) {
          const multiplier = this.getDurationMultiplier(noteDurations[this.currentNoteIndex]);
        ...
        this.playbackInterval = setTimeout(playNextNote, duration * 1000);
      };
  ...
  getDurationMultiplier(durationType) {
    switch (durationType) {
